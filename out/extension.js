...
function activate(context) {
    ...
    const decodeAtCursorDisposable = vscode.commands.registerCommand('protobuf-tool.decodeMessageAtCursor', () => {
        ...
        const allMessageNames = new Set();
        for (const line of lines) {
            const m = line.match(messageDefRegex);
            if (m) {
                allMessageNames.add(m[1]);
            }
        }
        ...
        if (wordUnderCursor && allMessageNames.has(wordUnderCursor)) {
            ProtoDecoderPanel_1.ProtoDecoderPanel.createOrShow(editor.document.uri.fsPath, wordUnderCursor);
            return;
        }
        ...
        if (!messageName) {
            vscode.window.showWarningMessage('Could not find a message definition at or above the cursor. Place your cursor inside a message block and try again.');
            return;
        }
        ProtoDecoderPanel_1.ProtoDecoderPanel.createOrShow(editor.document.uri.fsPath, messageName);
    });
